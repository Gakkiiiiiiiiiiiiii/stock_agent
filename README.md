...
- 行情层默认接入 AkShare / 东方财富实时 A 股数据，失败时自动回退到本地样例源。
...
默认环境变量中的 `MARKET_DATA_PROVIDER=akshare`，会优先拉取真实 A 股行情、市场快照和行业强度；若外部源异常，会自动回退到本地样例数据，接口仍可用。
