...
def retrieve_memory(query: str, filters: dict | None = None, top_k: int = 5, memory_types: list[str] | None = None, market_regime: str | None = None) -> dict:
    candidate_k = max(top_k * 5, 30) if memory_types else top_k
    result = HybridRetriever().retrieve(query=query, task_type="memory_lookup", filters=filters, top_k=candidate_k)
