...
if (Test-Path -LiteralPath ".\.conda-env\python.exe") {
    $pythonExecutable = (Resolve-Path ".\.conda-env\python.exe").Path
...
Write-Host "  `"$pythonExecutable`" scripts/check_skill_contracts.py"
Write-Host "  `"$pythonExecutable`" -m pytest -q -m `"not integration and not media and not qmt and not slow`""
Write-Host "  `"$pythonExecutable`" -m pytest tests/architecture -q"
Write-Host ""
