...
def test_migration_readiness_profiles_and_shared_token_are_explicit():
    ...
    for name in services:
        assert set(services[name]["profiles"]) == {"fixture", "bilibili", "xiaoe", "full-model"}
...
def test_fixture_stack_is_secret_safe_and_worker_is_confined():
    ...
    assert services["agent-api"]["environment"]["KNOWLEDGE_CONCLUSION_MODEL_ADAPTER"] == "fixture"
    assert services["agent-api"]["environment"]["KNOWLEDGE_CONCLUSION_FIXTURE_MODEL_URL"] == "http://fixture-model:9000/v1/chat/completions"
