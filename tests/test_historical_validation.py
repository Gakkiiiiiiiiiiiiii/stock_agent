...
def test_persisted_historical_validator_checks_coverage_and_future_data(isolated_database):
    ...
    day = date(2026, 8, 10)
    repo.save_market_snapshot("CN_A", datetime(2026, 8, 10, 15), day, "v1", {}, quality_flags=[])  # noqa: DTZ001 - sqlite DateTime contract is naive
    repo.save_sector_snapshot("银行", day, datetime(2026, 8, 10, 15), {}, .5, "v1", coverage=1.0)  # noqa: DTZ001 - sqlite DateTime contract is naive
    ...
    repo.save_market_snapshot("CN_A", datetime(2026, 8, 11, 9), day, "v1", {}, quality_flags=["DATA_UNAVAILABLE"])  # noqa: DTZ001 - sqlite DateTime contract is naive
    report = validate_history(day, day, {day})
